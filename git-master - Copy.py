...
def create_repo_buttons(root):
    for repo in repos:
        frame = tk.Frame(root)
        frame.pack(pady=10)
        label = tk.Label(frame, text=repo, width=20, anchor='w')
        label.pack(side="left")
        push_button = tk.Button(frame, text="git-push", command=lambda repo=repo: run_git_command(repo, "push"))
        push_button.pack(side="left", padx=5)
        pull_button = tk.Button(frame, text="git-pull", command=lambda repo=repo: run_git_command(repo, "pull"))
